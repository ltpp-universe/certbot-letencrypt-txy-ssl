...
async function addDnsRecord(params) {
  ...
  const fileName = `certbot_txy_record_${params.domain}_${params.validation}`;
  const recordIdFile = path.join(os.tmpdir(), fileName);
...
async function removeDnsRecord(params) {
  ...
  console.log(`Reading record ID from file: ${recordIdFile} -> ${recordId}`);
 ...
 * @param {string} [token] ACME challenge token
 * @returns {string} Record ID file path
 */
function getRecordIdFilePath(domain, token) {
  const fileName = token
    ? `certbot_txy_record_${domain}_${token}`
    : `certbot_txy_record_${domain}`;
